...
const CircuitPage = () => {
  ...
  const [showExBrowser, setShowExBrowser] = useState(false); // Full exercise browser for swap
  const [browserExId, setBrowserExId] = useState(null); // which exercise to replace
  const [expandedCats, setExpandedCats] = useState(new Set(['Gambe'])); // open categories in browser
  ...
  // Generate a circuit on demand - picks one exercise per category, randomly shuffled
  const generateCircuit = useCallback(() => {
    if (exercises.length === 0) return;
    setGenerating(true);
    // Pick one exercise per category, then fill to 5-6 total
    const categories = [...new Set(exercises.map(e => e.categoria).filter(Boolean))];
    const picks = [];
    // One per category (shuffled within each)
    categories.forEach(cat => {
      const catExs = exercises.filter(e => e.categoria === cat).sort(() => Math.random() - 0.5);
      if (catExs.length > 0) picks.push(catExs[0]);
    });
    // Shuffle picks and take up to 6
    const selected = picks.sort(() => Math.random() - 0.5).slice(0, 6);
    const newLogs = {};
    selected.forEach((ex) => {
      const exId = ex.exercise_id || ex.nome;
      const numSerie = ex.serie_default || 3;
      const reps = ex.ripetizioni_default || 12;
      const peso = ex.peso_default || 0;
      const sets = [];
      for (let i = 0; i < numSerie; i++) {
        sets.push({ set_number: i + 1, ripetizioni: reps, peso_kg: peso, completato: false });
      }
      newLogs[exId] = {
        nome: ex.nome, exercise_id: exId, categoria: ex.categoria || '',
        descrizione: ex.descrizione_tecnica || '', note: ex.note_sicurezza || '',
        sets, piano_serie: numSerie, piano_ripetizioni: reps, piano_peso_kg: peso,
      };
    });
    setLogs(newLogs);
    setGenerating(false);
  ...
  // Open full exercise browser for swap
  const openExBrowser = (exId) => {
    setBrowserExId(exId);
    setShowExBrowser(true);
  };
  ...
  return (
    <div className="min-h-screen bg-background pb-24" data-testid="circuit-page">
        ...
        {status === 'idle' && (
          <div className="space-y-3">
            ...
            >
              ...
              {generating ? 'Generazione...' : 'Genera nuovo circuito'}
            </button>
          </div>
        )}
        {status === 'active' && (
                ...
                <div className="px-4 pb-4 space-y-2">
                  ...
                  {status === 'idle' && (
                    <div className="flex gap-2">
                      <button 
                        onClick={(e) => { e.stopPropagation(); openExBrowser(exId); }}
      ...
      {/* Full Exercise Browser Modal for Swap */}
      {showExBrowser && (() => {
        const currentExName = logs[browserExId]?.nome;
        const categories = [...new Set(exercises.map(e => e.categoria))].filter(Boolean);
        const byCategory = {};
        categories.forEach(cat => {
          byCategory[cat] = exercises.filter(e => e.categoria === cat);
        });
        return (
          <div className="fixed inset-0 bg-black/70 z-50 flex items-end justify-center" onClick={() => setShowExBrowser(false)}>
            <div className="bg-surface border-t border-border rounded-t-3xl w-full max-w-md flex flex-col" style={{ maxHeight: '80vh' }} onClick={e => e.stopPropagation()}>
              {/* Header sticky */}
              <div className="sticky top-0 bg-surface p-4 border-b border-border flex items-center justify-between flex-shrink-0">
                <div>
                  <h3 className="text-text-primary font-bold">Scegli esercizio</h3>
                  <p className="text-text-secondary text-sm">Sostituisci: {currentExName}</p>
                </div>
                <button onClick={() => setShowExBrowser(false)} className="text-text-secondary"><X size={20} /></button>
              </div>
              {/* Scrollable body */}
                ...
                {categories.map(cat => {
                  ...
                  return (
                    <div key={cat} className="border border-border rounded-2xl overflow-hidden">
                      {/* Category header */}
                      <button
                        className="w-full flex items-center justify-between px-4 py-3 bg-surface-highlight text-left"
                        onClick={() => {
                      ...
                      >
                        <span className="text-text-primary font-bold text-sm">{cat}</span>
                        <div className="flex items-center gap-2">
                          <span className="text-text-secondary text-xs">{catExercises.length} esercizi</span>
                      ...
                      {isOpen && (
                        <div className="divide-y divide-border">
                          {catExercises.map((ex, i) => {
                            const isCurrent = (ex.exercise_id || ex.nome) === browserExId;
                            return (
                              <button
                                key={i}
                                disabled={isCurrent}
                                onClick={() => swapExercise(browserExId, ex)}
                                className={`w-full px-4 py-3 text-left transition-colors ${isCurrent ? 'opacity-40 cursor-not-allowed bg-surface' : 'hover:bg-primary/10'}`}
                                data-testid={`browser-ex-${i}`}
                              >
                                <p className="text-text-primary text-sm font-medium">{ex.nome}{isCurrent && ' (attuale)'}</p>
                                <p className="text-text-secondary text-xs mt-0.5">
                                  {ex.serie_default}x{ex.ripetizioni_default} · {ex.peso_default > 0 ? `${ex.peso_default}kg` : 'Corpo libero'}
                                </p>
                                {ex.descrizione_tecnica && (
                                  <p className="text-text-secondary text-xs mt-1 line-clamp-2 opacity-70">{ex.descrizione_tecnica}</p>
                                )}
                              </button>
                            );
                          })}
