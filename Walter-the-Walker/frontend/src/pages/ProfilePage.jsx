...
const isGuest = (user) => user?.user_id?.startsWith('guest_') || !user?.email || user?.email?.includes('@guest.local');
  ...
  return (
    <div className="min-h-screen bg-background pb-24" data-testid="profile-page">
      ...
      <div className="px-6 space-y-3">
        ...
        {isGuest(profile) && (
          <div className="bg-surface border border-border rounded-3xl p-5">
            <p className="text-text-primary font-bold mb-1">Accedi con Google</p>
            <p className="text-text-secondary text-sm mb-3">
              Salva i tuoi progressi su tutti i dispositivi collegando il tuo account Google.
            </p>
            <a
              href="/api/auth/google/login"
              className="flex items-center justify-center gap-3 w-full h-12 bg-white text-gray-800 font-bold rounded-2xl hover:bg-gray-100 transition-colors text-sm"
